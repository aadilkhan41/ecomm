...
import { Search, ShoppingCart, MapPin, Menu, ChevronDown, Check, Phone, X, Star, Tag, Award, ArrowLeft } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 py-3 md:py-4">
        <div className="relative min-h-[48px] flex items-center">
          ...
          <div className={`w-full flex items-center justify-between gap-4 transition-all duration-300 ${isMobileSearchOpen ? 'opacity-0 invisible scale-95 pointer-events-none md:opacity-100 md:visible md:scale-100 md:pointer-events-auto' : 'opacity-100 visible scale-100'}`}>
            ...
            {/* Search Bar - Desktop Only (re-uses styling) */}
          ...
          {/* Mobile Full Search Bar Overlay (Replacing brand row when open) */}
        ...
        <div className="fixed inset-0 z-[60] md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[80%] max-w-sm bg-white shadow-2xl flex flex-col animate-in slide-in-from-left duration-300">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white">
              <h2 className="text-2xl font-bold tracking-tighter cursor-pointer" onClick={() => { onLogoClick(); setIsMobileMenuOpen(false); }}>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-livBlue via-livPurple to-primary">liv</span>
                <span className="text-primary">.</span>
              </h2>
              <button 
            ...
            <div className="flex-1 overflow-y-auto py-4">
              ...
              <div className="px-6 pt-4 border-t border-gray-100">
                <button 
                  onClick={() => { onCustomerServiceClick(); setIsMobileMenuOpen(false); }}
                  className="w-full flex items-center gap-3 px-8 py-4 text-gray-600 hover:text-primary transition-colors font-bold text-sm"
                >
